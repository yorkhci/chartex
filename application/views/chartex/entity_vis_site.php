...
//GENERATES JIT DATA VISUALISATION
////////////////////////////////////////////////////////
	...
	echo heading('Site Visualisation', 3);
	echo '<p>Below is a visualisation of the transactions that '.$title.' <em>possibly</em> appears in:</p>';
	...
	foreach($possibly_entities as $row) {
		...
		if ($row['document_id'] != $current_doc) {
			...
			$entity_tree_root['children'][] = $doc_array;
			$current_subj = '';	
		}	
		...
		if ($row['entity_id'] != $current_subj) {			
			...
			end($entity_tree_root['children']);	
			$subj_key = key($entity_tree_root['children']);
			$entity_tree_root['children'][$subj_key]['children'][] = $rel_array; 	
			$current_rel = '';	
		}
