<?php
	...
	foreach($sameas_entities as $row) {
		...
		if ($row['document_id'] != $current_doc) {
			...
			//JSON array
			$top_array['label'] = '<span class="document_link">'.$row['document_name'].'</span> <span class="entity_type">('.$row['collection_name'].')</span>';					
	...
	foreach($possibly_entities as $row) {
		...
		if ($row['entity_id'] != $current_subject) {
			...
			<br /><span class="entity_type">(Confidence = '.$confidence.'%)</span>';	
			$middle_array['href'] = 'document/'.$row['document_id'];
			$middle_array['title'] = 'Document '.$row['document_id'];
			$middle_array['data-tabinfo'] = $row['document_id'].'~'.$row['document_name'].'~'.$row['entity_id'];
			$middle_array['id'] = 'entity~'.$row['entity_id'];
		...
		if ($row['relation_name'] != "same_as") {	
			...
			if ($row['subject_entity_id'] == $row['entity_id']) {		
				//JSON array
				end($maybe_array[$middle_key]['children']);
				$bottom_key = key($maybe_array[$middle_key]['children']);				
				...
				$bottom_array['id'] = 'entity~'.$row['object_entity_id'];
				$bottom_array['class'] = 'entity_link';
				$maybe_array[$middle_key]['children'][$bottom_key]['children'][] = $bottom_array;	 	
...
if (isset($maybe_array)){
	...
	echo '</script>';	
	echo '<p>The following documents contain references to entities that are <em>possibly</em> the same '.strtolower($possibly_entities[0]['entity_type_name']).':</p>';
